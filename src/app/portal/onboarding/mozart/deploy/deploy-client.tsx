...
import { deployPlan, deployApprove, refreshRun, submitRequirementSet, approveRequirementSet, upsertGuardrails, getRequirementSet } from "@/lib/api";
...
export default function DeployTimelineClient() {
  ...
  async function doPlan() {
    ...
    try {
      ...
      let numBudget = Number(budget);

      if (!numBudget || Number.isNaN(numBudget) || numBudget <= 0) {

        // Try to fetch budget from backend snapshot if not in local state
        try {
          const rs = await getRequirementSet(requirementSetId);
          const fetchedBudget =
            (rs as any)?.business_context_json?.budget_eur_month ??
            (rs as any)?.business_context?.budget_eur_month ??
            (rs as any)?.guardrails?.budget_eur_month;
          numBudget = Number(fetchedBudget);
        } catch {
          // ignore fetch failure, will fall back to redirect below
        }
      }

      if (!numBudget || Number.isNaN(numBudget) || numBudget <= 0) {

        setError("Budget is required. Go to Cost & Governance and set budget_eur_month.");

        router.push(`/portal/onboarding/mozart/cost-governance?requirement_set_id=${requirementSetId}`);
      ...
      // Only auto-retry for "not approved" / conflict style errors.
      // Do NOT retry for guardrails_failed (user must fix inputs).
      if (
        !lower.includes("guardrails failed") &&
        !lower.includes("guardrails_failed") &&
        (msg.includes("409") || lower.includes("requirement_set_not_approved"))
      ) {
        try {
  ...
  return (
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      {error ? (
        ...
        <pre className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700 whitespace-pre-wrap">{error}</pre>
